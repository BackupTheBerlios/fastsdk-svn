...
public abstract class AbstractTransferDropTargetListener implements
     ...
     * <code>null</code>, {@link #createTargetRequest()} is called and the
     * newly created Request is returned.
     * 
     ...
     * calculated by comparing the event's
     * {@link DropTargetEvent#dataTypes dataTypes} with the
     * <code>Transfer's</code> supported types ({@link Transfer#isSupportedType(TransferData)}).
     * If a dataType is supported, an attempt is made to find a <i>target</i>
     * <code>EditPart</code> at the current drop location. If a target
     * <code>EditPart</code> is found, <code>true</code> is returned, and
     * the DropTargetEvent's {@link DropTargetEvent#currentDataType} is set to
     ...
     * {@link #isEnabled(DropTargetEvent)}. For most DND operations, the data
     * is not available, thus asking for a command would not make sense. The
    ...
    /**
     * Sets the <i>target</i> <code>EditPart</code>. If the target is
     * changing, {@link #handleExitingEditPart()} is called before the target
     * changes, and {@link #handleEnteredEditPart()} is called afterwards.
     * 
     * @param ep
