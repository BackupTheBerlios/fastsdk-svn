...
import org.fast.fastsdk.core.model.commands.ChangeGuideCommand;
import org.fast.fastsdk.core.model.commands.CloneCommand;
import org.fast.fastsdk.core.model.commands.FastAddCommand;
import org.fast.fastsdk.core.model.commands.FastCreateCommand;
import org.fast.fastsdk.core.model.commands.SetConstraintCommand;

/**
 * @author …Ú»›÷€
 */
public class FastXYLayoutEditPolicy extends XYLayoutEditPolicy {

    public FastXYLayoutEditPolicy(XYLayout layout) {
        super();
        setXyLayout(layout);
    }

    protected Command chainGuideAttachmentCommand(Request request,
            FastSubpart part, Command cmd, boolean horizontal) {
        Command result = cmd;
    ...
    protected Command createAddCommand(Request request, EditPart childEditPart,
            Object constraint) {
        FastSubpart part = (FastSubpart) childEditPart.getModel();
        Rectangle rect = (Rectangle) constraint;

        FastAddCommand add = new FastAddCommand();
        add.setParent((FastDiagram) getHost().getModel());
        add.setChild(part);
        add.setLabel("add node");
        add.setDebugLabel("FastXYEP add subpart");//$NON-NLS-1$

        SetConstraintCommand setConstraint = new SetConstraintCommand();
        setConstraint.setLocation(rect);
        setConstraint.setPart(part);
        setConstraint.setLabel("add node");
        setConstraint.setDebugLabel("FastXYEP setConstraint");//$NON-NLS-1$
    ...
    protected Command createChangeConstraintCommand(
        ...
        FastSubpart part = (FastSubpart) child.getModel();
        cmd.setPart(part);
    ...
    protected EditPolicy createChildEditPolicy(EditPart child) {
        ...
        return new FastResizableEditPolicy();
    }
    ...
    protected FastGuide findGuideAt(int pos, boolean horizontal) {
        ...
        return (FastGuide) provider.getGuideAt(pos);
    }
    ...
    protected Command getCloneCommand(ChangeBoundsRequest request) {
        ...
        clone.setParent((FastDiagram) getHost().getModel());
        ...
        while (i.hasNext()) {
            ...
            clone.addPart((FastSubpart) currPart.getModel(),
    ...
    protected Command getCreateCommand(CreateRequest request) {
        FastCreateCommand create = new FastCreateCommand();
        create.setParent((FastDiagram) getHost().getModel());
        FastSubpart newPart = (FastSubpart) request.getNewObject();
        create.setChild(newPart);
        Rectangle constraint = (Rectangle) getConstraintFor(request);
        create.setLocation(constraint);
        create.setLabel("create node");
